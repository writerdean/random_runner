...
function getRandomCard() {
  ...
  const cardImage = (card.hasOwnProperty('image_url')) ? `<img src=${card['image_url']} alt="">` : `<img src=https://netrunnerdb.com/find/?q=${card['title'].split(' ').join('+').toLowerCase()}>`;
...
function displayCorpDeck (deck) {
  const type = deck[1]["type"];
  const content = document.getElementById('identity-content');
  const ident = deck[0];
  const card =   ident.hasOwnProperty('image_url') ?  `<img src="${deck[0]['image_url']}" alt="">` : `<img src="https://netrunnerdb.com/card_image/${ident['code']}.png" alt="">`;
  ...
  // each 'i' is an array of the types, including type name, length, and array of cards
    ...
    for (let x = 1; x < deck[i]['array'][0].length; x++) {
      const arrayGrouped = groupBy(deck[i]['array'][0], 'title');
      const titles = Object.keys(arrayGrouped);

      titles.forEach(function (item) {
        let content = document.getElementById(`${arrayGrouped[item][0]["type_code"]}-content`);
        let card = (arrayGrouped[item][0]['type_code'] == 'identity') 
    ...
    let typeCountContainer = document.getElementById(`${type}-count`);
